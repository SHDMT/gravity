...
import (
	...
	"github.com/SHDMT/gravity/platform/smartcontract/vm/lispvm"
)
...
func TestSystemContract(t *testing.T) {
	...
	db, err := setupEnv("./~temp/TestSystemContract")
	if err != nil {
		t.Fatal(err)
	}
	...
	for index := 0; index < len(cons); index++ {
		...
		lv := lispvm.NewLispVM(context, vm.Config{Mode: vm.VMModeContract})
...
func TestLockOutput(t *testing.T) {
	...
	db, err := setupEnv("./~temp/TestLockOutput")
	if err != nil {
		t.Fatal(err)
	}
	...
	restricts := []hash.HashType{rests[0].CalcAddress(), rests[1].CalcAddress()}
	consAddr := cons[1].CalcAddress()
	in1, utxo1 := FakeContractInput(1000, userEntries[1].address, inputParams[0], prevOutParams[12], nil)
	in2, utxo2 := FakeContractInput(2000, userEntries[2].address, inputParams[1], prevOutParams[1], nil)
	out1 := FakeContractOutput(3000, userEntries[3].address, outputParams[6], restricts)
	ins := []*structure.ContractInput{in1, in2}
	outs := []*structure.ContractOutput{out1}

	unit, err := FakeUnitWithInOut(0, consAddr, ins, outs, userEntries)
	if err != nil {
		t.Error(err.Error())
	}
	//PrintUnit(unit)
	...
	context := &messagevalidator.Context{
		TxUtxoEntry:     utxos,
		Msg:             unit.Messages[0],
		...
		CurrentMsgIndex: 0,
		MCI:             10,
		//FetchUnitByHash func(hashType hash.HashType)*structure.Unit
	}
	ret := messagevalidator.ValidateMessage(structure.InvokeMessageType, context)
	if !ret {
		t.Error("validate message should return false.")
	}

	utxo := out1.ToUtxo(assetHash, unit.Hash(), 0, 0)
	utxos = utxos[:0]
	utxos = append(utxos, utxo)

	in1 = FakeContractInputFromUTXO(utxo, inputParams[0])
	out1 = FakeContractOutput(3000, userEntries[3].address, outputParams[6], restricts)
	ins = []*structure.ContractInput{in1}
	outs = []*structure.ContractOutput{out1}
	unit, err = FakeUnitWithInOut(0, consAddr, ins, outs, userEntries)
	if err != nil {
		t.Error(err.Error())
	...
	ret = messagevalidator.ValidateMessage(structure.InvokeMessageType, context)
	if !ret {
		t.Error("Can't validate message.")
	}

	tx.Commit()
}

func TestRestrictOutput(t *testing.T) {
	_, err := FakeUnitWithSig()
	if err != nil {
		t.Fatal(err)
	}
	db, err := setupEnv("./~temp/TestRestrictOutput")
	if err != nil {
		t.Fatal(err)
	}
	...
	restricts := []hash.HashType{rests[0].CalcAddress(), rests[1].CalcAddress()}
	consAddr := cons[1].CalcAddress()
	in1, utxo1 := FakeContractInput(1000, userEntries[1].address, inputParams[0], prevOutParams[12], restricts)
	in2, utxo2 := FakeContractInput(2000, userEntries[2].address, inputParams[1], prevOutParams[1], nil)
	out1 := FakeContractOutput(3000, userEntries[3].address, outputParams[6], restricts)
	ins := []*structure.ContractInput{in1, in2}
	outs := []*structure.ContractOutput{out1}
...
func FakeRestrict(name string, code []byte) *structure.Contract {
	...
	contract.Name = name
	contract.ScriptCode = lispvm.LispVMVersion
...
func FakeContractInputFromUTXO(utxo *structure.TxUtxo, inParams map[string][]byte) *structure.ContractInput {
	input := structure.NewContractInput()
	for k, v := range inParams {
		input.AddParam(k, v)
	}
	input.SourceUnit = utxo.Unit
	input.SourceMessage = utxo.Message
	input.SourceOutput = utxo.Output
...
func setupEnv(dbPath string) (database.Db, error) {
	//dbPath := "./~temp/"
	os.RemoveAll(dbPath)
...
func FakeSystemRestricts() ([]*structure.Contract, error) {
	...
	for i, file := range files {
		...
		contract := FakeRestrict(file.Name(), buf)
