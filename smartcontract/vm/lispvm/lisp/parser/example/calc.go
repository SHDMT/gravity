...
//Calc is an interpreter on "+ - * /"
//This function is just a test implementation which is useless for the whole lisp interpretation
func Calc(s string) (float64, error) {
	...
	scanner := express.NewScanner([]byte(s), true)
	for {
		a, b, c := scanner.Scan()
		if c != nil {
			break
